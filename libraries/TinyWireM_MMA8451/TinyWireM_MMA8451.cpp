...
/**************************************************************************/
/*!
    @brief  Abstract away platform differences in Arduino TinyWireM library
*/
/**************************************************************************/
...
//undocumented version of requestFrom handles repeated starts on Arduino Due
#ifdef __SAM3X8E__
    TinyWireM.requestFrom(_i2caddr, 1, reg, 1, true);
#else
    ...
    TinyWireM.beginTransmission(_i2caddr);
    i2cwrite(reg);
    TinyWireM.endTransmission(false); // MMA8451 + friends uses repeated start!!
    TinyWireM.requestFrom(_i2caddr, 1);
#endif
    
    if (! TinyWireM.available()) return -1;
    return (i2cread());
}
...
void Adafruit_MMA8451::read(void) {
  ...
  x = TinyWireM.read(); x <<= 8; x |= TinyWireM.read(); x >>= 2;
  y = TinyWireM.read(); y <<= 8; y |= TinyWireM.read(); y >>= 2;
  z = TinyWireM.read(); z <<= 8; z |= TinyWireM.read(); z >>= 2;
